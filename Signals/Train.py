...
class MyNet(nn.Module):
    def __init__(self, window=10, subwindow=5, dropout=0):
        super().__init__()
        ...
        # c1 = nn.Sequential(
        #     nn.Conv1d(1, 3, 5, padding=4), nn.MaxPool1d(2), nn.SELU())

        # c2 = nn.Sequential(
        #     nn.Conv1d(3, 3, 3, padding=2, dilation=2), nn.MaxPool1d(2),
        #     nn.SELU())
            
        # # c3 = nn.Sequential(
        # #     nn.Conv1d(10, 10, 3, padding=2, dilation=2), nn.MaxPool1d(2),
        # #     nn.SELU())

        # self.feature_extractor = nn.Sequential(c1, c2)

        self.classifier = nn.Sequential(
            nn.Linear(10, 3), nn.SELU(), nn.AlphaDropout(dropout),
            nn.Linear(3, 1)
        )

        self.regressor = nn.Sequential(
            nn.Linear(10, 3), nn.SELU(), nn.AlphaDropout(dropout),
            nn.Linear(3, 1), nn.Hardtanh(locn_start, locn_end)
        )
...
def train(model: nn.Module,
    ...
    for data, target in train_data:
        ...
        # print("reconstructions loss =", reconstruction_loss)
...
def test(model: nn.Module,
    ...
    for data, target in test_loader:
        ...
        loss_mse = (distance * peak_indicator_target -
                    peak_distance_target * peak_indicator_target)**2
        loss_mse = loss_mse.sum() / (peak_indicator_target.sum() + 1e-10)
